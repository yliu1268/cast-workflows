...
Example command:  python extract_sample.py --name extract_sample_aou --vcf gs://fc-aou-datasets-controlled/v7/wgs/short_read/snpindel/acaf_threshold_v7.1/vcf/acaf_threshold.chr21.vcf.bgz
...
current location: $WORKSPACE_BUCKET/tr_imputation/tr_imputation/sample/aou_sample_list.txt
"""
...
def RunWorkflow(json_file, json_options_file, dryrun=False):
	...
	"""
	cmd = "cromshell submit ../wdl/extract_sample.wdl {json} -op {options}".format(json=json_file, options=json_options_file)
...
def main():
	...
	json_dict = {}
	json_dict["extract_sample.vcf"] = args.vcf
	json_dict["extract_sample.vcf_index"]= args.vcf+".tbi"
	json_dict["extract_sample.out_prefix"] = args.name
	json_dict["extract_sample.GOOGLE_PROJECT"] = project
	json_dict["extract_sample.GCS_OAUTH_TOKEN"] = token
